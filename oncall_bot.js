...
var mentionOnCalls = function(channel, message) {
  ...
  getOnCallSlackers(function(slackers) {
    ...
    bot.postMessageToChannel(channel, usersToMention.trim() + ", " + message, {
      icon_emoji: iconEmoji,
    });
  });
};
...
var getUser = function(findBy, value = "", callback) {
  if (findBy == FIND_BY_EMAIL && value.indexOf("@") > 0) {
    ...
    findBy == FIND_BY_NAME &&
    !(value.indexOf("U") == 0 && value.length == 9)
  ) {
...
bot.on("message", function(data) {
  ...
  if (data.type == "message") {
    ...
    if (
      (notABot || data.bot_id != bot.self.id) &&
      ...
      getChannel(data.channel, function(channel) {
        ...
        if (channel) {
          ...
          if (message.match(new RegExp("^" + botTag + ":? who$"))) {
            ...
            // default
            preText =
              (data.user ? " <@" + data.user + ">" : botTag) + ' said _"';
            if (botTagIndex == 0) {
              mentionOnCalls(
                channel.name,
